...
As the table demonstrates, I will compare the time-for-change model to my previous best model, a linear regression model based on average support. I will evalaute each model's predictive ability of an incumbent paty candidate's two-party popular vote share based on their in-sample fit and out-of-sample errors, with the variables in the table giving insight into each of these metrics. 
